...
export function createPrismaConversationRepository(prisma:PrismaClient):ConversationRepository{return{
 async getOrCreate(externalId,customerId=null){const x=await prisma.conversation.upsert({where:{externalId},create:{externalId,customerId},update:customerId?{customerId}:{}});return map(x);},
