...
export function createPrismaOrderRepository(prisma:PrismaClient):OrderRepository{return{
 ...
 async create(i){try{
  ...
  const prepared=i.items.map((x,n)=>{const p=products[n];if(!p)throw new Error("product_not_found");const v=x.variantId?p.variants.find(v=>v.id===x.variantId):undefined;if(x.variantId&&!v)throw new Error("variant_not_found");if(!v&&p.variants.length)throw new Error("variant_required_for_stock_control");return{productId:p.id,variantId:v?.id??null,productName:p.name,sku:v?.sku??null,size:v?.size??null,color:v?.color??null,unitPrice:v?.priceOverride??p.price,quantity:x.quantity};});
  const row=await prisma.$transaction(async tx=>{
   for(const item of prepared){if(!item.variantId)continue;const changed=await tx.productVariant.updateMany({where:{id:item.variantId,productId:item.productId,stockQuantity:{gte:item.quantity}},data:{stockQuantity:{decrement:item.quantity}}});if(changed.count!==1)throw new Error("insufficient_stock");}
