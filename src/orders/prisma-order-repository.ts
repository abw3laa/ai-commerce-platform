...
export function createPrismaOrderRepository(prisma:PrismaClient):OrderRepository{return{
 ...
 async create(i){try{
  ...
  if(!i.items.length||i.items.some(x=>!Number.isInteger(x.quantity)||x.quantity<=0))throw new Error("invalid_order_items");
  const prepared=i.items.map((x,n)=>{const p=products[n];if(!p)throw new Error("product_not_found");const v=x.variantId?p.variants.find(v=>v.id===x.variantId):undefined;if(x.variantId&&!v)throw new Error("variant_not_found");if(!v)throw new Error("variant_required_for_stock_control");return{productId:p.id,variantId:v.id,productName:p.name,sku:v.sku,size:v.size??null,color:v.color??null,unitPrice:v.priceOverride??p.price,quantity:x.quantity};});
  const row=await prisma.$transaction(async tx=>{
   for(const item of prepared){const changed=await tx.productVariant.updateMany({where:{id:item.variantId,productId:item.productId,stockQuantity:{gte:item.quantity}},data:{stockQuantity:{decrement:item.quantity}}});if(changed.count!==1)throw new Error("insufficient_stock");}
   return tx.order.create({data:{orderNumber:i.orderNumber,customerId:i.customerId,status:"received",items:{create:prepared.map(x=>({product:{connect:{id:x.productId}},variant:{connect:{id:x.variantId}},productName:x.productName,sku:x.sku,size:x.size,color:x.color,unitPrice:x.unitPrice,quantity:x.quantity}))}},include:{items:true}});
  });
