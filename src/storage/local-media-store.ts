...
export interface MediaStore{save(input:{stream:NodeJS.ReadableStream;extension:string;maxBytes:number}):Promise<{key:string;sizeBytes:number}>;open(key:string):NodeJS.ReadableStream;remove(key:string):Promise<void>;size(key:string):Promise<number>;}
