...
export function validateMediaSignature(mime:string,buffer:Buffer):boolean{const signature=SIGNATURES.find(x=>x.mime===mime);return !!signature&&signature.matches(buffer);}
