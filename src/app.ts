...
export async function buildApp(env:Env=loadEnv(),deps?:AuthRepositories):Promise<FastifyInstance>{
 ...
 if(!deps){
  ...
  await app.register(adminShippingRoutes,{prefix:"/admin",deps:resolvedDeps,authorizationRepo,shipmentRepo,tracker:env.SHIPPING_TRACKING_BASE_URL&&env.SHIPPING_TRACKING_API_KEY?createHttpShipmentTracker({baseUrl:env.SHIPPING_TRACKING_BASE_URL,apiKey:env.SHIPPING_TRACKING_API_KEY}):undefined,isProduction:env.NODE_ENV==="production"});
