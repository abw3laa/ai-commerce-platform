...
export async function adminCatalogRoutes(app:FastifyInstance,o:AdminCatalogOptions){
 ...
 app.get("/media/:id",{preHandler:[auth,guard]},async(req,reply)=>{const m=await o.mediaRepo.get((req.params as any).id);if(!m)return reply.code(404).send({error:"not_found"});reply.type(m.mimeType);return reply.send(o.mediaStore.open(m.storageKey));});
 async function upload(req:any,reply:any,expected:MediaOwner){const id=(req.params as any).id as string;const q=req.query&&typeof req.query==="object"?req.query as Record<string,unknown>:{};const mime=typeof q.contentType==="string"?q.contentType:"";const ext=allowed.get(mime);if(!ext)return reply.code(415).send({error:"unsupported_media_type"});const kind=mime.startsWith("image/")?"image":"video";const name=typeof q.filename==="string"&&q.filename.trim()?q.filename.trim():`upload.${ext}`;try{const saved=await o.mediaStore.save({stream:req.body as any,extension:ext,maxBytes:max});const media=await o.mediaRepo.create({ownerType:expected,ownerId:id,kind,originalName:name,mimeType:mime,storageKey:saved.key,sizeBytes:saved.sizeBytes});return reply.code(201).send(media);}catch(e){if(e instanceof Error&&e.message==="media_too_large")return reply.code(413).send({error:e.message});throw e;}}
 ...
 app.delete("/media/:id",{preHandler:[auth,guard]},async(req,reply)=>{const m=await o.mediaRepo.delete((req.params as any).id);if(!m)return reply.code(404).send({error:"not_found"});await o.mediaStore.remove(m.storageKey);return {deleted:true};});
}
